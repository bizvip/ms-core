...
{
    public function __construct(private readonly Builder|ModelBuilder $builder, private readonly array $params,) { }
    ...
    {
        return $this->where(
            $column, function (Builder|ModelBuilder $builder) use ($keys, $column) {
    ...
    public function datetimeRange(string $column, array|string $keys): self
    {
        return $this->where(
            ...
            return $builder->whereBetween(
                $column, [
                    $start instanceof CarbonInterface ? $start
                        : Carbon::createFromFormat(CarbonInterface::DEFAULT_TO_STRING_FORMAT, $start),
                    $end instanceof CarbonInterface ? $end
                        : Carbon::createFromFormat(CarbonInterface::DEFAULT_TO_STRING_FORMAT, $end),
                ]
            );
        }, ''
        );
    ...
    public function dateRange(string $column, array|string $keys): self
    {
        return $this->where(
            ...
            return $builder->whereBetween(
                $column, [
                    $start instanceof CarbonInterface ? $start->startOfDay()
                        : Carbon::createFromFormat('Y-m-d', $start)->startOfDay(),
                    $end instanceof CarbonInterface ? $end->endOfDay()
    ...
    {
        return $this->where(
            $column, function (Builder|ModelBuilder $builder, mixed $value) use ($operator, $column, $next) {
            if ($next) {
